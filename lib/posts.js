...
function parseDateField(dateField) {
  if (dateField instanceof Date) {
    return Number.isNaN(dateField.getTime()) ? null : dateField;
  }
...
async function readPostFromDisk(fileName) {
  ...
  const parsedDate = parseDateField(data.date);
  const rawDate = parsedDate
    ? parsedDate.toISOString().slice(0, 10)
    : typeof data.date === 'string'
      ? data.date
      : '';
  const dateISO = parsedDate ? parsedDate.toISOString() : '';
